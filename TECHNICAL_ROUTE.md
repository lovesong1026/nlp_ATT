...
```text
data/original_question.md（完整原句，唯一事实来源）
        │
        ├── segmentation_words.txt（最小分词词典，freq=2）
        ▼
┌───────────────────────────────────────────────────────────────┐
│ DEP 原始句法事实                                              │
│ cws + dep；保存完整依存图和全部原始 ATT，不筛选、不修复         │
└──────────────────────────────┬────────────────────────────────┘
                               ▼
┌───────────────────────────────────────────────────────────────┐
│ 原子 ATT 筛选、纠错与补召回                                   │
│ LTP/base2；cws + pos + dep + sdp + srl                        │
│ 输出：第四阶段原子ATT（1476条）                               │
└──────────────────────────────┬────────────────────────────────┘
                               │
...
┌───────────────────────────────────────────────────────────────┐
│ 维度过滤                                                      │
│ 修饰词或中心词命中已删除维度时，删除整条原子关系                │
│ 输出：第四阶段去除维度原子ATT（897条）                         │
└──────────────────────────────┬────────────────────────────────┘
                               ▼
┌───────────────────────────────────────────────────────────────┐
│ 无 LLM 规则合并                                               │
│ 原子图连续合并 + SRL动词定语合并 + 属性—实体合并               │
│ 有合并则输出合并关系；无合并则回填去维度原子ATT                 │
│ 输出：规则合并结果（145句触发合并，共159条合并关系）            │
└──────────────────────────────┬────────────────────────────────┘
                               ▼
┌───────────────────────────────────────────────────────────────┐
│ LLM 输入汇合                                                  │
│ 完整原句 + 维度禁止区间 + DEP/原子ATT/SRL/规则合并证据         │
└──────────────────────────────┬────────────────────────────────┘
                               ▼
┌───────────────────────────────────────────────────────────────┐
│ qwen-plus LLM 业务语义裁决                                    │
│ 识别业务实体；筛选、修剪、合并、补漏；不改写原句                │
└──────────────────────────────┬────────────────────────────────┘
                               ▼
┌───────────────────────────────────────────────────────────────┐
│ 程序硬校验                                                    │
│ Pydantic Schema + 批次ID + 原文连续跨度 + 不重叠               │
│ + 标点边界 + 维度禁止区间 + 禁止时间/数量/空泛谓词              │
└──────────────────────────────┬────────────────────────────────┘
                               ▼
                 可用于业务 RAG 的完整定语关系
```
...
1. 原句是唯一事实来源。
2. DEP 原始事实保持可审计；原子 ATT 处理负责筛选、纠错和补召回；
   无 LLM 规则只做确定性合并。
3. POS、DEP、SDP、SRL 和规则合并结果都是可错、可缺失的候选证据。
4. LLM 负责业务实体识别与业务语义裁决，不负责创造原句中不存在的文本。
5. 程序负责 Schema、原文跨度和维度区间等确定性约束，不能把硬约束
   全部交给提示词。
6. SDP 只以受约束关系参与原子 ATT 修复；NER、SDPG 不进入当前正式
   处理链路。

当前状态说明：

- DEP、原子 ATT、维度过滤和无 LLM 规则合并已经按上图运行并生成
  全量结果；
- LLM代码和提示词已经存在，但输入适配仍使用DEP输出重命名前的
  `最终原子ATT`列名。正式串联前需改为当前的`DEP原始ATT`，并决定是否
  将`规则合并结果`作为新增候选证据；在完成该适配前，第5阶段不属于
  当前可直接全量运行的闭环。
...
当前 DEP 原始事实脚本使用`LTP/base`；原子 ATT 抽取和无 LLM
规则合并默认使用：
...
第四阶段主流程保持原子关系可审计；维度过滤和第4.1阶段规则合并通过后续
列展示，不会覆盖`第四阶段原子ATT`。规则合并不使用LLM，也不承担最终
业务价值判断。

### 8.1 第4.1阶段：无 LLM 规则合并
...
规则合并只接收`第四阶段去除维度原子ATT`及第四阶段已有SRL证据：

- 原子图连续合并；
- SRL完整动词性定语与连续实体中心合并；
- 属性修饰语与紧凑实体中心合并；
- 不补词、不调序、不改写，不重新引入被删除维度；
- 没有触发合并时，原样回填去维度原子ATT。
...
这些输入都只是证据，不是标准答案。LLM 可以接受、拒绝、修剪和合并候选，
也可以直接从原句补充候选未覆盖的关系。当前代码仍使用`A/S/V`候选编号；
接入现有第三、四和4.1阶段输出时，需要同步更新列名映射，并为规则合并结果
增加独立来源标识。
